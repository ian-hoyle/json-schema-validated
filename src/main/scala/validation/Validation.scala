...
object Validation {
  ...
  /** Validates the provided data using a sequence of fail-fast validations followed by composed validations.
    ...
    *   `Seq[List[RowData] => DataValidationResult[List[RowData]]]` - A sequence of validations to apply in order, stopping at the first failure.
    * @param composeValidations
    *   `Seq[List[RowData] => DataValidationResult[List[RowData]]]` - A sequence of validations to apply after fail-fast validations, combining their results.
    * @return
